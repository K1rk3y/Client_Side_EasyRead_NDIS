...
from core.generate_images import generate_images_from_prompts  # Import your function
from core.generate_images import generate_images_from_prompts
from core.pdf_generation import compile_info_for_pdf, update_text, caller
from core.word_generation import create_docx
from core.word2pdf import convert_to_pdf
...
def process_pdf(pdf_file_path):
    ...
    def progress_callback(current_image, total_images):
        ...
        UPLOAD_PROGRESS["progress"] = int(
            round(progress_start + (current_image * progress_increment))
        )

    # Generate images using the results as prompts
    generated_images, docx_results = generate_images_from_prompts(
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and user.check_password(form.password.data):
            ...
            return (
                redirect(next_page) if next_page else redirect(url_for("index.process"))
            )
        else:
...
def process():
    ...
    if request.method == "POST" and form.validate_on_submit():
        ...
        return render_template(
            "processing.html"
        )  # Render a template that shows the progress bar
...
def upload_template():
    if request.method == "POST":
        ...
        print(
            f"DEBUG: Conversion option received: {conversion_option}"
        )  # Debug statement
...
def display():
    ...
    if TOTAL_PAGES is None or page_text_boxes is None:
        TOTAL_PAGES, page_text_boxes, all_groups, mapping = compile_info_for_pdf(
            results, generated_images, template, temp_file_path
        )
    else:
