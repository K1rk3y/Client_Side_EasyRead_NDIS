...
def split_into_sentences(text):
    sentences = [
        sentence.strip() + "." for sentence in text[1:].split(".") if sentence.strip()
    ][:-1]
    return sentences
...
def summarise(pdf_path):
    ...
    array_2d = [
        split_into_sentences(translate(" ".join(group))) for group in grouped_paragraphs
    ]
    ai_summary = [item for sublist in array_2d for item in sublist]

    # 保存预处理的文本到media/text/output.txt
    output_dir = os.path.join(settings.MEDIA_ROOT, "text")
