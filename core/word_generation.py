...
def estimate_row_height(
    ...
    column_width_in,
    line_spacing=1,
    font_path="app/static/test_fonts/calibri.ttf",
):
    """
    ...
    column_width_pt = (
        column_width_in - 0.16
    ) * 80  # 80 DPI (dots per inch), 0.08 inch is default side margin.
    ...
    num_lines = len(lines)
    height_pt = (
        num_lines * (font_size * (line_spacing))
    ) * 1.24  # 1.24 is factor for line spacing
...
def create_page(
    doc, content_list, boxes_per_page, page_width, page_height, x_margin, y_margin
):
    """
    ...
    for content in content_list:
        if i == boxes_per_page:
            ...
            if total_height + tmp > space_height_inch - 0.2 * (
                i + 1
            ):  # Check if the latest box can fit with a minimum margin of 0.2.
                break
            else:
                total_height += tmp
                i += 1
    ...
    for content in content_list[0:i]:
        ...
        add_content(
            doc, content["image_path"], content["text"], image_width, text_width
        )
    return i


def create_document(
    output_path, content_list, boxes_per_page
):  # header_text = "Header text", header_image = "test_images/logo.png", footer_text = "Footer Text", footer_image = "test_images/logo.png"
    """
    ...
    while i < len(content_list):
        n = create_page(
            doc,
            content_list[i : i + boxes_per_page],
            boxes_per_page,
            page_width,
            page_height,
            x_margin,
...
def create_document_template(
    output_path, content_list, boxes_per_page, template_path="intelife-template.docx"
):
    """
    ...
    y_margin = (
        ...
        + doc.sections[0].header_distance.inches
        + doc.sections[0].footer_distance.inches
    )
