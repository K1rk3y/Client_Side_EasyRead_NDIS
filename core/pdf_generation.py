...
def generate_pdf(output_path, template_pdf, all_groups, groups_per_page):
    try:
        ...
        print(
            f"Warning: Could not open template PDF. Using default page size. Error: {e}"
        )
        page_width, page_height = (
            595,
            842,
        )  # Default size: A4 (in points, 72 points per inch)
    ...
    while i < len(all_groups):
        ...
        max_height = (
            ...
            - MARGIN_TOP
            - MARGIN_BOTTOM
            - MINIMUM_VERTICAL_MARGIN * (groups_per_page - 1)
        )
        n = add_groups(
            new_page, all_groups[i : i + groups_per_page], groups_per_page, max_height
        )
